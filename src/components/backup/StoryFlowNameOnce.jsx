...
export default function StoryFlow({
  ...
  // Resolve a stable key for dialogue speaker comparison
  const getSpeakerKey = (block) => {
    if (!block || block.type !== "characterDialogue") return null;
    const ch = block.character;
    if (typeof ch === "string") return ch;
    if (ch && typeof ch === "object" && ch.id) return ch.id;
    return null;
  };
    ...
    >
      <div className="story-flow-content has-scroll" ref={contentRef}>
        {renderedBlocks.map((block, i) => {
          ...
          const prevBlock = i > 0 ? renderedBlocks[i - 1] : null;
          const speakerKey = getSpeakerKey(block);
          const prevSpeakerKey = getSpeakerKey(prevBlock);
          const hideSpeakerLabel =
            speakerKey && prevSpeakerKey && speakerKey === prevSpeakerKey;
